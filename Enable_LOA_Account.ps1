...
Function EnableAccount{
  param($username)
    try{
        # Enable ActiveSync Devices tied to the Account
        $MobileDevices = Get-MobileDevice -Mailbox $username
        foreach($device in $MobileDevices){
          Set-CASMailbox -Identity $username -ActiveSyncAllowedDeviceIDs $device.DeviceId
          write-host "Enabled $device.DeviceId from connecting with ActiveSync" -ForegroundColor Green
        }
        write-host "Enabling Exchange ActiveSync Devices was successful" -ForegroundColor Green

    }catch{
        write-host "Enabling Exchange ActiveSync Devices Failed, Please log into ECP and do it manually!!!" -ForegroundColor Red
    }

    #Enable ActiveSync, OWA, OWA for Devices
    try{
        Set-CASMailbox -Identity $username -ActiveSyncEnabled $true
        write-host "Enabling Exchange ActiveSync was successful" -ForegroundColor Green
        Set-CASMailbox -Identity $username -OWAforDevicesEnabled $true
        write-host "Enabling Exchange OWA for Devices was successful" -ForegroundColor Green
        Set-CasMailbox -Identity $username -OWAEnabled $true
        write-host "Enabling Exchange OWA was successful" -ForegroundColor Green
    }catch{
        write-host "Enabling Exchange ActiveSync, OWA for Devices, or OWA has Failed, Please log into ECP and do it manually!!!" -ForegroundColor Red
    }
}

Function EnabledExchangeAccounts{
    ...
    if($eol -eq $true){
        ...
        EnableAccount -username "$username@shastahealth.org"
        Remove-PSSession $Session
    }else{
...
Function EnableUserAccount{
    ...
    try{
        ...
        write-host "Active Directory Account has been enabled." -ForegroundColor Green
    }catch{
        write-host "Failed to enable Active Directory Account. Please do it manually!!!" -ForegroundColor Red
    }
}
...
Function ConfirmEnableCloud{
    ...
    Do{
     ...
     }While($user.BlockCredential -eq $false)
...
$userAcct = Read-Host -Prompt "What Username do you want to enable?"
...
if($confirmation.ToUpper() -eq "Y" -and $userAcct -ne $null){
    ...
    ImportADCommands -Server "DC01" -creds $creds
    ...
    If($UserLocation -eq $true){
        ...
        EnabledExchangeAccounts -username $userAcct -eol $false -UserCredential $creds
    }else{
        # User is in the cloud
        write-host "Users Mailbox is in the Cloud" -ForegroundColor Yellow
        EnabledExchangeAccounts -username $userAcct -eol $true -UserCredential $o365creds
    }

    #Disable AD Account
    EnableUserAccount -username $userAcct
	 
    # Syn no matter where Account is to disable account online services
    Write-Host "Performing a Sync to Azure to enable Account in Cloud Servies..." -ForegroundColor Green
    # Start a Azure Sync
    #SyncAD -creds $o365creds -ADConnectServer $ADConnectServer
		
    $ADDisableResult = ConfirmEnableCloud -username $userAcct
    if($ADDisableResult -eq $true){
        write-host "Sync Completed. Account $userAcct has been enabled in the cloud." -ForegroundColor Green
    }else{
        write-host "Sync Failed. Please enable account $userAcct manually and rerun a sync to Azure." -ForegroundColor Red
    }
    #MoveDisabledUser -username $userAcct -credentials $creds -Path $DisabledOU # Still need to ad exporting of Email before move.
}else{
    write-host "Enabling Canceled!!" -ForegroundColor Red
}